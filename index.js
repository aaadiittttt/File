/*
Credits 
A BOT V2 made by Adit
ig : adiittt17_
gh : aaadiittttt
thanks to GPTAsisstant, GeminiAI, ChatGPT
*/
...
try{
		if (connection === 'close') {
			...
			if (reason === DisconnectReason.badSession) {
				...
				NawBotz();
			} else putu.end(`Unknown DisconnectReason: ${reason}|${connection}`)
		}
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
            ...
            console.log(color(`🌿Connected to => ` + JSON.stringify(putu.user, null, 2), 'yellow'))
			await delay(1999)
            console.log(chalk.yellow(`\n\n               ${chalk.bold.blue(`[ Adittt ]`)}\n\n`))
            console.log(color(`< ================================================== >`, 'cyan'))
	        console.log(color(`\n${themeemoji} IG: adiittt17_`,'magenta'))
            console.log(color(`${themeemoji} GITHUB: aaadiittttt `,'magenta'))
            console.log(color(`${themeemoji} WA NUMBER: 6285757571072`,'magenta'))
            console.log(color(`${themeemoji} CREDIT: Adittt17\n`,'magenta'))
		}
...
require('./case')(putu, m, chatUpdate, store)
} catch (err) {
console.log(err)}})
...
app.listen(port, () => {
  ...
  NawBotz(); // Mulai bot setelah server berjalan
});
